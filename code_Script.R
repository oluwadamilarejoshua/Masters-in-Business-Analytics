...
rail_data <- read.csv("dataToAnalyze.csv", header = T, na.strings = c("", " "))
str(rail_data)
view(rail_data)
...
# season_diff <- aov(EEE_Season ~ NRR, data = test_data)
# summary(season_diff)
# 
# interchange_diff <- aov(Interchange_Station1819 ~ NRR, data = test_data)
# summary(interchange_diff)
